...
export function SyncHistoryTab({
  ...
  const [viewingRow, setViewingRow] = useState<QuickbooksSyncHistoryRow | null>(
    null
  );
  const {
    page,
    ...
    total,
    sliced,
  } = useTableState<QuickbooksSyncHistoryRow>({
    data: list,
    pageSize: 10,
    searchKeys: ["syncMessage"],
    searchFn: (item, s) =>
      (item.syncMessage ?? "").toLowerCase().includes(s),
  ...
  return (
    <div className="p-6">
      ...
      <p className="text-sm text-muted-foreground mb-4">
        Runs when you use &quot;Sync from QuickBooks&quot; (import) or when
        transactions are pushed to QuickBooks.
      </p>
      <SearchAndDateFilter
        search={search}
            ...
            {sliced.length === 0 ? (
              ...
              sliced.map((row) => (
                <TableRow key={row.id}>
                  <TableCell>
                    {row.syncStartTime
                      ? new Date(row.syncStartTime).toLocaleString()
                      : "—"}
                  </TableCell>
                  <TableCell>
                    {row.syncEndTime
                      ? new Date(row.syncEndTime).toLocaleString()
                      : "—"}
                  </TableCell>
                  <TableCell>
                    {row.status === 0
                  ...
                  <TableCell>
                    <Button
                      ...
                      disabled={
                        !row.syncedEntities ||
                        Object.keys(row.syncedEntities).length === 0
                      }
                    >
      ...
      >
        ...
        <div className="space-y-2 text-sm">
          ...
          {viewingRow?.syncedEntities &&
            Object.keys(viewingRow.syncedEntities).length === 0 && (
              <p className="text-muted-foreground">
                No entity counts recorded.
              </p>
            )}
        </div>
      </Modal>
