...
import { type CompanyDetails } from '@/api/kycService'

interface CompanyDetailTabProps {
  company: CompanyDetails
}

const ShadowCard: React.FC<{ children: React.ReactNode; className?: string }> = ({ children, className }) => (
  <div className={`bg-white rounded-2xl shadow-sm border border-gray-100 ${className}`}>
    {children}
  </div>
)

const CompanyDetailTab: React.FC<CompanyDetailTabProps> = ({ company }) => {
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <ShadowCard className="p-4 border border-gray-100">
        <div className="flex items-start justify-between">
          <div className="flex items-center space-x-5">
            <div className="p-3 rounded-2xl text-blue-600 shadow-sm border border-blue-100">
              <Building2 size={25} />
            </div>
            <div>
              <h2 className="text-3xl font-semibold tracking-tight">{company.name}</h2>
              <div className="flex items-center space-x-2 mt-1">
                <p className="text-xs font-medium uppercase tracking-widest text-gray-500">Registration Number :</p>
                <span className="font-mono text-sm font-semibold bg-gray-50 px-2 py-0.5 rounded border border-gray-100">
      ...
      {((company.shareClasses && company.shareClasses.length > 0) || (company.issuedShares || 0) > 0) && (
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
            {company.shareClasses?.map((share, index) => (
              <ShadowCard key={index} className="p-4 rounded-xl border-gray-100 flex items-center justify-between">
                <p className="text-[14px] font-medium uppercase tracking-widest group-hover:text-blue-600">
                ...
                <div className="flex flex-col items-end">
                  ...
                  {share.perShareValue != null && Number(share.perShareValue) > 0 && (
                    <p className="text-[15px] text-gray-400 font-medium border border-gray-400 rounded-lg px-3 py-1">
                      €{Number(share.perShareValue).toFixed(2)} / share
                    </p>
                  )}
                </div>
              </ShadowCard> 
              ...
              <ShadowCard className="bg-gray-50 p-4 rounded-xl border-gray-100 flex items-center justify-between border hover:bg-white hover:shadow-md transition-all group">
                ...
                <div className="flex flex-col items-end">
                  <p className="text-[14px] font-medium tracking-wide">
                    {((company.issuedShares || 0) - (company.shareClasses?.reduce((acc, s) => acc + s.issued, 0) || 0)).toLocaleString()}
                  </p>
                </div>
              </ShadowCard>
            )}
          ...
          <div>
            ...
            <p className="text-gray-900 text-xl font-light">
              {company.companyStartedAt
                ? new Date(company.companyStartedAt).toLocaleDateString() 
                : 'N/A'}
            </p>
          </div>
        </ShadowCard>
