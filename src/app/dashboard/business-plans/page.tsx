...
// Business Plan Project Interface
...
export default function BusinessPlansPage() {
  const [activeProject, setActiveProject] = useState<BusinessPlanProject | null>(null);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    // Load active business plan project
    loadActiveProject();
  }, []);
  ...
  const loadActiveProject = async () => {
    ...
    try {
      ...
      // const project = await fetchBusinessPlanProject();
    ...
    <section className="mx-auto max-w-[1400px] w-full pt-5 space-y-6">
      ...
      {loading ? (
        <DashboardCard className="p-12 text-center">
          <p className="text-muted-foreground">Loading business plan projects...</p>
        </DashboardCard>
      ) : activeProject ? (
        <>
            ...
            <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
              <div className="flex-1">
                <div className="flex items-center gap-3 mb-3">
                  <h2 className="text-2xl font-bold text-brand-body">
                    BUSINESS PLAN — {getPurposeLabel(activeProject.purpose).toUpperCase()}
                  </h2>
                  <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold ${
                    activeProject.status === "waiting_on_you" 
                ...
                <div className="grid md:grid-cols-3 gap-6 mt-5">
                  <div>
                    ...
                    <p className="font-semibold text-brand-body">{getPurposeLabel(activeProject.purpose)}</p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground uppercase tracking-wider mb-1.5">Expected delivery</p>
                    <p className="font-semibold text-brand-body">{activeProject.expected_delivery || "TBD"}</p>
                  </div>
                  <div>
                    <p className="text-xs text-muted-foreground uppercase tracking-wider mb-1.5">Prepared by</p>
                    <p className="font-semibold text-brand-body">{activeProject.prepared_by}</p>
                  </div>
                </div>
              </div>
              <div className="flex flex-col gap-2.5 shrink-0">
                ...
                <Link href={`/dashboard/business-plans/${activeProject.project_id}`}>
                  <Button variant="default" className="w-full md:w-auto min-w-[160px]">
                    <Eye className="w-4 h-4 mr-2" />
                    View progress
                  </Button>
                </Link>
          ...
          <DashboardCard className="p-6">
            ...
            <div className="grid md:grid-cols-2 gap-6">
              ...
              <div className="space-y-1">
                ...
                <p className="font-semibold text-brand-body">{activeProject.industry || "—"}</p>
              </div>
              <div className="space-y-1">
                <p className="text-xs text-muted-foreground uppercase tracking-wider mb-1.5">Forecast period</p>
                <p className="font-semibold text-brand-body">{activeProject.forecast_years} years</p>
              </div>
              <div className="space-y-1">
                <p className="text-xs text-muted-foreground uppercase tracking-wider mb-1.5">Prepared by</p>
                <p className="font-semibold text-brand-body">{activeProject.prepared_by}</p>
        ...
        <DashboardCard className="p-12 text-center">
          ...
          <h3 className="text-xl font-semibold text-brand-body mb-2">No active business plan projects</h3>
          <p className="text-muted-foreground mb-6">
            Request a new business plan to get started. Our team will guide you through the process.
          </p>
          <Link href="/dashboard/business-plans/request">
