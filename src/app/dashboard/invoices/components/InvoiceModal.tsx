...
const InvoiceModal = ({ invoice, onClose }: InvoiceModalProps) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/10 backdrop-blur-xs bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white border border-blue-200/50 rounded-[16px] shadow-sm w-full mx-auto transition-all duration-300 hover:shadow-md max-w-6xl relative overflow-y-auto max-h-[90vh]">
        ...
        <div className="pt-0 p-4">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-6 mb-6 text-sm">
            <div>
              <strong className="text-sky-800">Date:</strong> {invoice.txnDate?.slice(0, 10) || "-"}
            </div>
            <div>
              <strong className="text-sky-800">Due:</strong> {invoice.dueDate?.slice(0, 10) || "-"}
            </div>
            <div>
              <strong className="text-sky-800">Customer:</strong> {invoice.customerName || "-"}
            </div>
            <div>
              <strong className="text-sky-800">Total:</strong> €{invoice.totalAmount?.toFixed(2) || "0.00"}
            </div>
            <div>
              <strong className="text-sky-800">Balance:</strong> €{invoice.balance?.toFixed(2) || "0.00"}
            </div>
            <div>
              <strong className="text-sky-800">Status:</strong> {invoice.status || "-"}
            </div>
          </div>
          {/* Line Items */}
          <h3 className="text-lg leading-normal text-black capitalize font-medium mb-2">Line Items</h3>
          ...
          {/* Attachments */}
          <h3 className="text-lg leading-normal text-black capitalize font-medium mb-2">Attachments</h3>
